...
## Before: otoliths.csv (data), gcm_4_agepred.csv, gcm_4_coefs.csv,
##         gcm_4_conv.csv, gcm_4_curve.csv, gcm_4_residuals.csv (output)
## After:  gcm_4.pdf (report)
...
agepred <- read.taf("output/gcm_4_agepred.csv")
coefs <- read.taf("output/gcm_4_coefs.csv")
conv <- read.taf("output/gcm_4_conv.csv")
curve <- read.taf("output/gcm_4_curve.csv")
otoliths <- read.taf("data/otoliths.csv")
res <- read.taf("output/gcm_4_residuals.csv")

blue <- paste0(palette()[4], "b0")
red <- paste0(palette()[2], "b0")
black <- "#000000b0"

pdf("report/gcm_4.pdf", width=10, height=4)
par(mfrow=c(1,3))
