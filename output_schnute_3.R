## Extract Schnute case 3 results of interest, write TAF output tables

## Before: schnute_3_results.RData (model)
## After:  schnute_3_agepred.csv, schnute_3_coefs.csv, schnute_3_conv.csv,
##         schnute_3_curve.csv, schnute_3_residuals.csv,
##         schnute_3_sigma.csv (output)
...
## Read model results
load("model/schnute_3_results.RData")

## Extract coefficients
coefs <- as.data.frame(report[c("L1", "L2", "b", "sigma_min", "sigma_max")])
L1 <- report$L1
L2 <- report$L2
b <- report$b
...
t2 <- report$t2
Age <- seq(0, 4, 0.02)
Length <- schnute3_curve(Age, L1, L2, b, t1, t2)
...
## Export tables
write.taf(agepred, "output/schnute_3_agepred.csv")
write.taf(coefs, "output/schnute_3_coefs.csv")
write.taf(conv, "output/schnute_3_conv.csv")
write.taf(curve, "output/schnute_3_curve.csv")
write.taf(otofit, "output/schnute_3_otofit.csv")
write.taf(res, "output/schnute_3_residuals.csv")
write.taf(sigma, "output/schnute_3_sigma.csv")
