...
## Before: otoliths.csv (data), gcm_1_agepred.csv, gcm_1_coefs.csv,
##         gcm_1_conv.csv, gcm_1_curve.csv, gcm_1_residuals.csv (output)
## After:  gcm_1.pdf (report)
...
agepred <- read.taf("output/gcm_1_agepred.csv")
coefs <- read.taf("output/gcm_1_coefs.csv")
conv <- read.taf("output/gcm_1_conv.csv")
curve <- read.taf("output/gcm_1_curve.csv")
otoliths <- read.taf("data/otoliths.csv")
res <- read.taf("output/gcm_1_residuals.csv")

blue <- paste0(palette()[4], "b0")
red <- paste0(palette()[2], "b0")
black <- "#000000b0"

pdf("report/gcm_1.pdf", width=10, height=4)
par(mfrow=c(1,3))
plot(NA, xlim=c(0,4), ylim=c(20,85), xlab="Age (yr)", ylab="Length (cm)")
title(main=paste0("L0 = ", round(curve$Length[curve$Age == 0]),
